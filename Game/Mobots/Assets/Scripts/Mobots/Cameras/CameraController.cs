...
namespace Mobots.Cameras {
	...
	public class PositionSettings {
		...
		[HideInInspector] public float newDistance = -6f; // set by the zoom input
		[HideInInspector] public float mAdjustmentDistance = -6f;
	}
	...
	public class CameraController : MonoBehaviour {
		...
		/// Input variables
		/// </summary>
		private float mMouseInputVertical,
			mMouseInputHorizontal,
			mZoomInput,
			mOrbitSnapInput;

		public void Initialize(Transform target, Robot.Player mPlayer = null) {
			if (mPlayer != null)
				mRobot = mPlayer;
		...
		void InitRotation() {
			...
			mDestination = Quaternion.Euler(mOrbit.mXRotation, mOrbit.mYRotation + mTarget.eulerAngles.y, 0) * -Vector3.forward *
			               mPosition.mDistanceFromTarget;
		...
		void Start() {
			if (!mTarget)
				...
				mRobot = (this.mTarget.GetComponent<Robot.Player>()) ? this.mTarget.GetComponent<Robot.Player>() : null;
				if (!mRobot)
					Debug.LogWarning("You dont have a robot script for your camera");
			}
			this.mCollision.Initialize(this.GetComponent<Camera>());
			this.mCollision.UpdateCameraClipPoints(this.transform.position, this.transform.rotation,
				ref this.mCollision.mAdjustedCameraClipPoints);
			this.mCollision.UpdateCameraClipPoints(this.mDestination, this.transform.rotation,
				ref this.mCollision.mDesiredCameraClipPoints);
		}
		...
		void MoveToTarget() {
			...
			this.mDestination = Quaternion.Euler(this.mOrbit.mXRotation, this.mOrbit.mYRotation, 0) * -Vector3.forward *
			                    this.mPosition.mDistanceFromTarget;
			this.mDestination += this.mTarget.position;

			if (this.mCollision.mColliding) {
				this.mAdjustedDestination = Quaternion.Euler(this.mOrbit.mXRotation, this.mOrbit.mYRotation, 0) * Vector3.forward *
				                            this.mPosition.mAdjustmentDistance;
				this.mAdjustedDestination += this.mTargetPos;
				if (this.mPosition.mSmoothFollow) {
					// Use smoothdamp function
					this.transform.position = Vector3.SmoothDamp(this.transform.position, this.mAdjustedDestination, ref this.mCamVel,
						this.mPosition.mSmooth);
				} else {
				...
				if (this.mPosition.mSmoothFollow) {
					// Use smoothdamp function
					this.transform.position = Vector3.SmoothDamp(this.transform.position, this.mDestination, ref this.mCamVel,
						this.mPosition.mSmooth);
				} else {
		...
		public class CollisionHandler {
			...
			[HideInInspector] public bool mColliding = false;
			[HideInInspector] public Vector3[] mAdjustedCameraClipPoints;
			[HideInInspector] public Vector3[] mDesiredCameraClipPoints;
			Camera mCamera;
			...
			public void UpdateCameraClipPoints(Vector3 camPosition, Quaternion atRotation, ref Vector3[] intoArray) {
				...
				// top left
				intoArray[0] =
					(atRotation * new Vector3(-x, y, z)) + camPosition; // Added and rotated the point relative to the camera
				// top right
				intoArray[1] =
					(atRotation * new Vector3(x, y, z)) + camPosition; // Added and rotated the point relative to the camera
				// bottom left
